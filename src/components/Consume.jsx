...
export default function Consume() {
  return (
    <section className="consume sec" id="consume">
      ...
      <div className="consume-top">
        <div>
          <h2 className="rev">WE ARE <span className="out">ALL</span><br />CONSUMERS</h2>
          <p className="rev type" style={{ color: '#9fc0d6', fontSize: 13, maxWidth: 520, lineHeight: 1.7, marginTop: 14 }}>
            What kind of dining set defines me as a person? We used to read pornography. Now it was the Horchow Collection.
          </p>
        </div>
        <div className="mag rev">
          <div className="mag-img">
            <img src="img/own-you.jpg" alt="The things you own end up owning you" loading="lazy" />
            <div className="mag-masthead">möbel</div>
            <div className="mag-cover">
              <span className="l">The <b>nesting instinct</b> — what your sofa says about your soul</span>
              <span className="l">47 ways to define yourself by <b>furniture</b></span>
              <span className="l">Special: the clever coffee table that owns you back</span>
            </div>
            <div className="mag-price">
              <div className="bc">{Array.from({ length: 30 }, (_, i) => <i key={i} style={{ width: ((i * 53) % 3) + 1 + 'px' }} />)}</div>
              <div className="pp">$—.99 · FALL ISSUE</div>
            </div>
          </div>
        </div>
      </div>
